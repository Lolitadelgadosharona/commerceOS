...
const marketSource={...base,id:"32323232-3232-4232-8232-323232323232",name:"Reviewed customer research",platform:"reddit",source_type:"community",access_method:"manual",reliability_score:.8,status:"active"};
const marketSignal={...base,id:marketSignalId,source_id:marketSource.id,region:"US",category:"pet",signal_type:"customer_pain",title:"Pet heat discomfort is rising",description:"Owners describe recurring heat discomfort during summer travel.",trend_direction:"rising",confidence_score:.82,observed_at:"2026-08-23T12:00:00Z",status:"validated"};
const marketSignalEvidence={...base,id:"33333333-4444-4333-8333-333333333333",signal_id:marketSignalId,evidence_type:"reference",content_reference:"research:pet-heat-2026",strength_score:.86,captured_at:"2026-08-23T12:00:00Z"};
const marketCluster={...base,id:"34343434-3434-4434-8434-343434343434",name:"Summer pet comfort",category:"pet",confidence:.78,impact_score:72};
const demandSignal={...base,id:"35353535-3535-4535-8535-353535353535",source_domain:"intelligence",source_reference_id:marketSignalId,source_type:"research_analysis",source_reference:"research:pet-heat-2026",collection_method:"manual",evidence_origin:"reviewed research",confidence_basis:"Two corroborating observations",customer_segment:"Traveling pet owners",category:"pet comfort",problem_statement:"Pets experience heat discomfort during travel.",customer_language:"My dog overheats in the car",frequency:12,confidence:.8,evidence_count:2,status:"approved"};
const demandEvidence={...base,id:"36363636-3636-4636-8636-363636363636",demand_signal_id:demandSignal.id,source_type:"research_analysis",source_id:marketSource.id,source_reference:"research:pet-heat-2026",evidence_text:"Owners report recurring summer heat discomfort."};
const demandDashboard={organization_id:organizationId,draft_signals:0,signals_in_review:0,approved_signals:1,source_overview:[{source_type:"research_analysis",signal_count:1,evidence_count:2,average_confidence:.8,signal_percentage:100}],emerging_categories:[{category:"pet comfort",signal_count:1,total_frequency:12,average_confidence:.8}],customer_pain_clusters:[],emerging_demand_themes:[],predictive_indicators:[],emerging_customer_pains:[{demand_signal_id:demandSignal.id,category:"pet comfort",customer_segment:"Traveling pet owners",problem_statement:demandSignal.problem_statement,frequency:12,confidence:.8,evidence_count:2,source_type:"research_analysis",evidence_sources:["research:pet-heat-2026"],source_conversation_ids:[]}]};
const productHypothesis={...base,id:productHypothesisId,opportunity_id:marketId,name:"Portable pet cooling mat",description:"A product hypothesis responding to observed travel heat discomfort.",customer_problem:"Pets overheat during summer travel.",solution_description:"A portable reusable cooling surface.",target_customer:"Traveling pet owners",target_market:"US pet accessories",status:"evaluating",confidence_score:.76};
const productEconomics={...base,id:"37373737-3737-4737-8737-373737373737",product_id:productHypothesisId,selling_price:"39.00",estimated_product_cost:"12.00",estimated_shipping_cost:"5.00",payment_cost:"1.20",estimated_marketing_cost:"8.00",contribution_margin:"12.80",margin_percentage:"32.82",currency:"USD"};
const productSupplier={...base,id:"38383838-3838-4838-8838-383838383838",product_id:productHypothesisId,source_type:"manual",supplier_reference:"supplier-research:cooling-1",estimated_cost:"12.00",minimum_order_quantity:100,lead_time:"30 days",quality_notes:"Material testing is still required.",risk_level:"medium"};
const productRisk={...base,id:"39393939-3939-4939-8939-393939393939",product_id:productHypothesisId,risk_type:"quality",severity:"medium",description:"Cooling duration claims require evidence.",status:"open"};
const productScore={...base,id:"40404040-4040-4040-8040-404040404040",product_id:productHypothesisId,opportunity_score:72,margin_score:64,risk_score:58,competition_score:55,confidence_score:76,overall_score:66.4,formula_version:"product-investment-v1"};
const economicInputs=[
 {...base,id:"41414141-4141-4141-8141-414141414141",product_economics_id:productEconomics.id,metric:"estimated_shipping_cost",value:"0.0000",classification:"quoted",source:"supplier quote",confidence:.9,as_of:"2026-08-23T12:00:00Z",evidence_reference:"supplier:shipping-1",notes:null},
 {...base,id:"42424242-4242-4242-8242-424242424242",product_economics_id:productEconomics.id,metric:"customer_acquisition_cost",value:null,classification:"unknown",source:"not researched",confidence:null,as_of:null,evidence_reference:null,notes:"Missing before decision"},
];
...
const committeePacket={organization_id:organizationId,opportunity:marketOpportunity,market_evidence:[marketEvidence],market_signals:[{signal:marketSignal,evidence:[marketSignalEvidence]}],source_diversity:2,product_theses:[{hypothesis:productHypothesis,economics:productEconomics,economic_inputs:economicInputs,supplier_candidates:[productSupplier],risks:[productRisk],investment_score:productScore,product_truth_relationship_status:"no_canonical_relationship"}],investment_memo:memo,approval:approval(),decision_queue:decision,launch_readiness:readiness,supporting_case:[marketEvidence.evidence_summary,marketSignal.title],opposing_case:[productRisk.description,"Customer evidence is required"],missing_evidence:["Customer evidence is required"],decision_quality_warnings:[{code:"critical_economics_unknown",severity:"blocking",message:"Critical economic inputs are missing or UNKNOWN: customer_acquisition_cost"}],unavailable_sections:["product_truth_comparison"]};
...
createServer((request, response) => {
  ...
  if (url.pathname === `/api/v1/opportunities/${marketId}/committee-packet`) return json(200,{...committeePacket,approval:approval()});
  ...
  if(url.pathname===`/api/v1/market-signals/${marketSignalId}/opportunities`)return json(200,[{link_id:"43434343-4343-4343-8343-434343434343",signal_id:marketSignalId,opportunity_id:marketId,linked_at:base.created_at,opportunity:marketOpportunity}]);
  if(url.pathname===`/api/v1/market-clusters/${marketCluster.id}/signals`)return json(200,[{membership_id:"44444444-5555-4444-8444-444444444444",cluster_id:marketCluster.id,signal:marketSignal,evidence:[marketSignalEvidence]}]);
  if(url.pathname===`/api/v1/product-hypotheses/${productHypothesisId}`)return json(200,productHypothesis);
  const commerceLists={
    "/api/v1/market-sources":[marketSource],"/api/v1/market-signals":[marketSignal],"/api/v1/market-evidence":[marketSignalEvidence],"/api/v1/market-clusters":[marketCluster],"/api/v1/demand-signals":[demandSignal],"/api/v1/demand-signal-evidence":[demandEvidence],
    "/api/v1/product-hypotheses":[productHypothesis],"/api/v1/product-economics":[productEconomics],"/api/v1/product-economic-inputs":economicInputs,"/api/v1/supplier-candidates":[productSupplier],"/api/v1/product-risks":[productRisk],"/api/v1/product-investment-scores":[productScore],"/api/v1/products":[],"/api/v1/product-truth":[],
  };
  if(url.pathname in commerceLists&&request.method==="GET"){
    if(scenario==="commerce-partial"&&url.pathname==="/api/v1/market-evidence")return json(503,{error:"evidence_unavailable"});
    return json(200,scenario==="empty"?[]:commerceLists[url.pathname]);
  }
  if(url.pathname==="/api/v1/demand-intelligence-dashboard")return json(200,scenario==="empty"?{...demandDashboard,approved_signals:0,source_overview:[],emerging_categories:[],emerging_customer_pains:[]}:demandDashboard);
