...
export default async function ProductPage({params}:{params:Promise<{id:string}>}){const {id}=await params;if(!UUID.test(id))notFound();const context=await resolveExecutiveContext();if(!context.ok)return <DashboardConfigurationState message={context.error.message}/>;const detail=await getProductHypothesisDetail(id,context.data.organization_id);if(!detail.hypotheses.ok)return <DashboardConfigurationState message={detail.hypotheses.error.message}/>;if(!detail.hypothesis)notFound();return <ProductHypothesisDetail hypothesis={detail.hypothesis} economics={detail.economics} suppliers={detail.suppliers} risks={detail.risks} scores={detail.scores}/>}
