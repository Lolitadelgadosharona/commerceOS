...
export default async function ShopifyProductPage({params}:{params:Promise<{productId:string}>}){const {productId}=await params;if(!UUID.test(productId))notFound();const context=await resolveExecutiveContext();if(!context.ok)return <DashboardConfigurationState message={context.error.message}/>;const workspace=await getShopifyWorkspace(context.data.organization_id);if(!workspace.ok)return <DashboardConfigurationState message={workspace.error.message}/>;const connection=workspace.data.connections[0]??null;const result=await getShopifyReadiness(productId,context.data.organization_id,connection?.id);if(!result.ok)return <DashboardConfigurationState message={result.error.message}/>;return <ShopifyProductDetail item={result.data} connection={connection} publication={workspace.data.publications.find(x=>x.product_id===productId)??null}/>}
