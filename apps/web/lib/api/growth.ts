...
export type RevenueExperiment = Entity & {
  name: string;
  description: string;
...
export type GrowthProspect = Entity & {
  business_name: string;
  website: string | null;
  email: string | null;
...
export type Qualification = Entity & {
  candidate_id: string;
  score: number | null;
...
export type ResearchRun = Entity & {
  candidate_id: string;
  status: string;
  failure_reason: string | null;
...
export type ResearchResult = Entity & {
  research_run_id: string;
  summary: string;
  evidence_summary: Array<Record<string, unknown>>;
  potential_growth_issues: string[];
  confidence: number;
  missing_information: string[];
  risk: string[];
};
export type GrowthOpportunity = Entity & {
  prospect_id: string;
...
export type Assignment = Entity & {
  ...
  prospect_id: string;
  assigned_offer: string;
  assigned_message: string;
  result_status: string;
};
export type OutreachEvent = Entity & {
  prospect_experiment_link_id: string;
  outreach_draft_id: string | null;
  event_type: string;
  occurred_at: string;
  event_metadata: Record<string, unknown>;
};
export type RevenueOffer = Entity & {
  prospect_id: string;
...
export type GrowthDashboard = {
  ...
  active_revenue_experiments: number;
};
export type GrowthReadiness = {
  organization_id: string;
  growth_os: "ready" | "degraded";
  ai: "ready" | "not_configured" | "error";
  worker: "ready" | "degraded" | "offline";
  database: "ready" | "error";
  manual_send_mode: "active";
  external_connectors: "not_configured" | "governed_web_search";
  queued_research: number;
...
async function all<T>(
  ...
  return apiGet<T[]>(path, { organization_id });
}
...
export async function loadGrowthHome(organization_id: string) {
  const [
    ...
    all<RevenueExperiment>("/api/v1/revenue-experiments", organization_id),
    all<Candidate>("/api/v1/prospect-candidates", organization_id),
    all<GrowthProspect>("/api/v1/growth-prospects", organization_id),
    all<Approval>("/api/v1/approvals", organization_id),
    all<Revenue>("/api/v1/revenue-observations", organization_id),
    all<Cost>("/api/v1/cost-observations", organization_id),
    all<Profit>("/api/v1/contribution-profit", organization_id),
    all<Learning>("/api/v1/improvement-recommendations", organization_id),
    apiGet<GrowthDashboard>("/api/v1/growthos-dashboard", { organization_id }),
    apiGet<GrowthReadiness>("/api/v1/growth-operational-readiness", {
      organization_id,
    }),
  ]);
  return {
...
export async function loadGrowthWorkspace(
  ...
  const [
    ...
    all<Diagnosis>("/api/v1/growth-diagnoses", organization_id),
    all<Gift>("/api/v1/growth-gifts", organization_id),
    all<Outreach>("/api/v1/growth-outreach-drafts", organization_id),
    all<OutreachEvent>("/api/v1/outreach-tracking-events", organization_id),
    all<RevenueOffer>("/api/v1/revenue-offers", organization_id),
    all<Capability>("/api/v1/ai/model-capabilities", organization_id),
  ]);
  const resultEntries: ResearchResult[] = [];
  if (runs.ok)
    for (const run of runs.data.slice(0, 20)) {
      const result = await apiGet<ResearchResult | null>(
        `/api/v1/growth-business-research-runs/${run.id}/results`,
        { organization_id },
      );
      if (result.ok && result.data) resultEntries.push(result.data);
    }
  return {
    ...home,
    assignments,
