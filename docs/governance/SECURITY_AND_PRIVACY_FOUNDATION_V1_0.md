...
Sprint 034 resolves the application-layer limitation with verified, expiring, revocable bearer sessions and a universal authentication, organization, RBAC, and domain-authorization boundary. `X-Actor-ID` is no longer a production identity source. Controlled external deployment still requires TLS, rate limiting, hardened production configuration, monitoring, and the operational controls defined in the [Production Authentication and Authorization Foundation](../security/PRODUCTION_AUTHORIZATION_FOUNDATION_V1_0.md).
