...
| Build | Approved products, append-only Product Truth, supporting product knowledge, brand claim guardrails, creative asset/version/performance records, and provider-neutral generation workflow | Governed product facts, Build lifecycle state, artifact registry, sequential asset versions, provider capability contracts, pending generation jobs, QA state, and supplied performance observations | Governance approvals, Intelligence hypotheses, supplier/payment truth, provider/model execution without a separately governed adapter, paid/channel execution, publishing, ledger truth | Decision, Growth, Operations, Governance, Learning |
